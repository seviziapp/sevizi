  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Search */}
        <Pressable style={[styles.search, shadow.sm]} onPress={() => router.push('/client/categories')}>
        ...
        {activeJob && (
          <Pressable style={[styles.jobBannerWrap, shadow.card]} onPress={() => router.push('/client/job-status')}>
            <LinearGradient colors={gradients.forest} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.jobBanner}>
        ...
        {/* CTA banner */}
        <Pressable style={[styles.bannerWrap, shadow.glow]} onPress={() => router.push('/client/new-request')}>
          <LinearGradient colors={gradients.mint} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.banner}>
            <View style={{ flex: 1 }}>
              <Text style={[text.h3, { color: colors.white }]}>Un besoin précis ?</Text>
              <Text style={[text.small, { color: 'rgba(255,255,255,0.85)', marginTop: 2 }]}>
                Publiez-le, recevez des offres en minutes.
              </Text>
            </View>
            <View style={styles.bannerBtn}>
              <Plus size={16} color={colors.vert} />
              <Text style={[text.small, { color: colors.vert, fontWeight: '600' }]}>Demander</Text>
            </View>
          </LinearGradient>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  jobBanner: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, padding: spacing.md },
  jobDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.vert },
  reqRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, backgroundColor: colors.white, borderRadius: radii.xl, padding: spacing.md, borderWidth: 1, borderColor: 'rgba(6,41,31,0.05)' },
  ...
  catIcon: { width: 56, height: 56, borderRadius: radii.xl, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center' },
  bannerWrap: { borderRadius: radii.xl, overflow: 'hidden' },
  banner: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.lg },
  bannerBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: colors.white, paddingHorizontal: spacing.lg, height: 40, borderRadius: radii.pill, justifyContent: 'center' },
});
