...
import { ArrowLeft, Wrench, MapPin, Navigation, Send, ShieldAlert } from 'lucide-react-native';
...
// Shared by both client and provider (no tab bar wrapper), so messaging works
// the same regardless of which side opens it.
export default function Thread() {
  const router = useRouter();
  const params = useLocalSearchParams<{ requestId?: string; otherName?: string }>();
  const [requestId, setRequestId] = useState<string | undefined>(params.requestId);
  const [otherName, setOtherName] = useState<string>(params.otherName ?? 'Sèvizi');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!rid) {
        ...
        if (active) { rid = active.requestId; if (!params.otherName) setOtherName(active.otherName); }
      }
      if (cancelled) return;
      setRequestId(rid);
      if (rid) {
  ...
  async function send() {
    ...
    // Keep contact info off the platform — see contentSafety.ts for why.
    if (containsContactInfo(body)) {
      setWarning('Merci de ne pas partager de numéro de téléphone ou d\'email. Toute la communication doit rester sur Sèvizi.');
      return;
    }
    setWarning('');
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
      ...
      {/* Persistent reminder: stay on-platform */}
      <View style={styles.safetyBanner}>
        <ShieldAlert size={14} color={colors.vertDark} />
        <Text style={[text.label, { color: colors.vertDark, flex: 1 }]}>
          Gardez toute communication et tout paiement sur Sèvizi. Ne partagez pas vos coordonnées personnelles.
        </Text>
      </View>
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            <Text style={[text.small, { color: colors.textMuted, textAlign: 'center' }]}>
              La messagerie s'ouvre une fois une mission en cours.{'\n'}Acceptez une offre pour discuter avec l'autre partie.
            </Text>
          </View>
        ) : msgs.length === 0 ? (
          <View style={styles.empty}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={styles.composer}>
          ...
          <TextInput
            ...
            value={draft}
            onChangeText={(v) => { setDraft(v); if (warning) setWarning(''); }}
...
const styles = StyleSheet.create({
  ...
  safetyBanner: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, backgroundColor: '#F2FBF6', paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.border },
  ...
  warningText: { color: colors.terre, fontSize: 13, textAlign: 'center', paddingHorizontal: spacing.lg, paddingTop: spacing.sm },
